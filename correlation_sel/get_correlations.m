function correlations = get_correlations(train_input_file,dev_input_file,model_save_dir,output_save_dir)
...
for i=1:length(sel_thresh)
	...
	% save the model
	save_file=strcat(model_save_dir,'/model_baseline_th_',num2str(thresh));
	save(save_file,'model');
	dev_output = svmpredict(dev_targets,cur_dev,model);

	% save the dev outputs
	dev_save=strcat(output_save_dir,'/output_baseline_th_',num2str(thresh));	
