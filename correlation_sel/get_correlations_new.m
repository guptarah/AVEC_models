function correlations = get_correlations_new(train_input_file,dev_input_file,model_save_dir,output_save_dir)
...
%for i=1:length(sel_thresh)
for i = 1:3
	thresh = sel_thresh(i);
	selected_indices = find(correlations>thresh);
	...
	% save the model
	save_file=strcat(model_save_dir,'/model_baseline_th_',num2str(thresh*100),'.mat');
	save(save_file,'model');
	dev_output = svmpredict(dev_targets,cur_dev,model);

	% save the dev outputs
	dev_save=strcat(output_save_dir,'/output_baseline_th_',num2str(thresh*100),'.mat');	
